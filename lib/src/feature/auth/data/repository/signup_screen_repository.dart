...
class SignupScreenRepository implements ISignup {
    ...
    Future<SignupUser> signup(SignupUser user) async {
        ...
        if (response.statusCode == 200 || response.statusCode == 201 ) {
            ...
            print( "AOOOOOOOOOOOOOOOOOOOBBBBAA");
            print( response.headers);
            final domain = SignupUser(user.username, user.birth, user.cep, user.email, 
                user.password, user.confirmPassword, user.intoleranciaAlimentar, user.restricaoAlimentar
                /*token: token*/);
            return Future.value(domain);
        } else {
          print( "EEEEHAAAAAAAAAAAA");
